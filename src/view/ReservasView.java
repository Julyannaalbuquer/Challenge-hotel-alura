...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(ReservasView.class.getResource("/assets/imagens/aH-40px.png")));
		...
		txtDataE.getCalendarButton().setIcon(new ImageIcon(ReservasView.class.getResource("/assets/imagens/icon-reservas.png")));
		...
		txtDataS.getCalendarButton().setIcon(new ImageIcon(ReservasView.class.getResource("/assets/imagens/icon-reservas.png")));
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				if (txtDataE.getDate() != null && txtDataS.getDate() != null) { 
					valorReserva = calcularValorReserva();
					if(valorReserva <= 0){
						txtValor.setText("Inválido");
					} else{
		...
		JLabel lblFormaPago = new JLabel("PAGAMENTO");
		...
		panel_1.add(logo);
		logo.setIcon(new ImageIcon(ReservasView.class.getResource("/assets/imagens/Ha-100px.png")));
		
		JLabel imagenFondo = new JLabel("");
		imagenFondo.setBounds(0, 140, 500, 409);
		panel_1.add(imagenFondo);
		imagenFondo.setBackground(Color.WHITE);
		imagenFondo.setIcon(new ImageIcon(ReservasView.class.getResource("/assets/imagens/reservas-img-3.png")));
		...
		btnProximo.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null && valorReserva > 0) {		
					registrarReserva();
					RegistroHospede registro = new RegistroHospede(reserva);
					registro.setVisible(true);
					dispose();
				} else {
					JOptionPane.showMessageDialog(null, "Campos vazios ou preenchimento incorreto");
				}
			}						
		});
	...
	private void registrarReserva(){
		reserva = new Reservas(txtDataE.getDate(), txtDataS.getDate(), valorReserva, txtFormaPagamento.getSelectedItem().toString());
		this.reservaController.registrarReserva(reserva);
	}

	private Double calcularValorReserva(){
		Integer diasEntreDatas = obterDiasEntreDatas(txtDataE.getDate(), txtDataS.getDate());
		return (double)80*diasEntreDatas;
	}

	private Integer obterDiasEntreDatas(Date dataInicial, Date dataFinal){
		long diferencaMilissegundos = dataFinal.getTime() - dataInicial.getTime();
		return (int)(diferencaMilissegundos/86400000);
	}
